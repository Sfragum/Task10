...
public class JwtService {
    ...
    // Blacklist refresh token for rotation
    private Set<String> blacklistedTokens = ConcurrentHashMap.newKeySet();

    public String generateToken(UserDetails userDetails) {
        return buildToken(userDetails, 15 * 60 * 1000); // 15 dakika
    }

    public String generateRefreshToken(UserDetails userDetails) {
        return buildToken(userDetails, 7 * 24 * 60 * 60 * 1000); // 7 gün
    }
    ...
    public String rotateRefreshToken(String oldToken, UserDetails userDetails) {
        // Eskisini kara listeye ekle
